  ...
  return (
    <div className="min-h-screen" style={{ background: 'var(--bg-1)' }}>
      ...
      <div className="pt-12 pb-6 px-5 text-center relative overflow-hidden">
        ...
        <button
          onClick={() => setDark(d => !d)}
          className="absolute top-4 right-4 w-9 h-9 flex items-center justify-center rounded-xl transition-all duration-200 hover:scale-110"
      ...
      <div className="max-w-5xl mx-auto px-4 pb-20">
        ...
        <div className="relative mb-6">
          ...
          <input
            ...
            className="w-full rounded-xl py-3 pl-9 pr-4 text-sm outline-none transition-colors"
            style={{
              background: 'var(--bg-2)',
          ...
          {filtered.map(role => {
            ...
            return (
              <div
                ...
                style={{
                  background: 'var(--bg-2)',
                  border: `1px solid ${pct > 0 ? role.color + '35' : 'var(--border)'}`,
                }}
              >
                {done > 0 && (
                  <button
                    onClick={e => { e.stopPropagation(); setConfirmRoleReset(role) }}
                    className="absolute top-3 right-3 w-7 h-7 flex items-center justify-center rounded-lg transition-all duration-200 hover:scale-110 z-10"
                    style={{ background: '#ef444415', border: '1px solid #ef444430', color: '#ef4444' }}
                    title={`Reset ${role.title} progress`}
                  >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4"
          ...
          <div className="rounded-2xl p-6 w-full max-w-sm fade-in"
            ...
            <p className="text-sm text-center mb-6 leading-relaxed" style={{ color: 'var(--txt-3)' }}>
              This clears all completed skills for this role across all 3 levels. Other roles are not affected.
            </p>
            <div className="flex gap-3">
              <button
                onClick={() => setConfirmRoleReset(null)}
              ...
              <button
                onClick={() => { resetRole(confirmRoleReset.id, confirmRoleReset.levels); setConfirmRoleReset(null) }}
                className="flex-1 py-2.5 rounded-xl text-sm font-bold transition-colors"
      ...
      {confirmReset && (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4"
          ...
          <div className="rounded-2xl p-6 w-full max-w-sm fade-in"
            ...
            <p className="text-sm text-center mb-6 leading-relaxed" style={{ color: 'var(--txt-3)' }}>
              This will clear every completed skill across all 13 roles. This cannot be undone.
            </p>
            <div className="flex gap-3">
              <button
